...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, net, predictions, targets, masks, num_crowds):
        ...
        # Mask IoU Loss
        if cfg.use_maskiou is not None:
        ...
        for idx in range(mask_data.size(0)):
            ...
            if cfg.mask_proto_double_loss:
                if cfg.mask_proto_mask_activation == activation_func.sigmoid:
                    if cfg.double_loss_type == 'huber_loss':
                        pre_loss = F.huber_loss(torch.clamp(pred_masks, 0, 1), mask_t, reduction='sum')
                    elif cfg.double_loss_type == 'log_cosh_bce_loss':
                        pre_loss = self.log_cosh_bce_loss(pred_masks, mask_t)
                    elif cfg.double_loss_type == 'dice_loss':
                        pre_loss = self.dice_loss(torch.clamp(pred_masks, 0, 1), mask_t)
                    elif cfg.double_loss_type == 'log_cosh_dice_loss':
                        pre_loss = self.log_cosh_dice_loss(torch.clamp(pred_masks, 0, 1), mask_t)
                    elif cfg.double_loss_type == 'mse_loss':
                        pre_loss = F.mse_loss(torch.clamp(pred_masks, 0, 1), mask_t, reduction='sum')
                    else:
            ...
            if cfg.mask_proto_mask_activation == activation_func.sigmoid:         
                if cfg.use_huber_loss:
                    pre_loss = F.huber_loss(torch.clamp(pred_masks, 0, 1), mask_t, reduction='none', delta=cfg.huber_loss_delta)
                elif cfg.use_log_cosh_bce_loss:
                    pre_loss = self.log_cosh_bce_loss(pred_masks, mask_t)
                elif cfg.use_dice_loss:
                    pre_loss = self.dice_loss(torch.clamp(pred_masks, 0, 1), mask_t)
                elif cfg.use_log_cosh_dice_loss:
                    pre_loss = self.log_cosh_dice_loss(torch.clamp(pred_masks, 0, 1), mask_t)
                elif cfg.use_mse_loss:
                    pre_loss = F.mse_loss(torch.clamp(pred_masks, 0, 1), mask_t, reduction='none')
                else:
    ...
    def dice_loss (self, input, target):
        ...
        dice = (2 *intersection + smooth)/(input + target + smooth)
        return 1-dice
        
    def log_cosh_dice_loss(self, input, target):
        x = self.dice_loss(input, target)
        return torch.log((torch.exp(x) + torch.exp(-x)) / 2.0)
        
    def log_cosh_bce_loss(self, input, target):
        x = F.binary_cross_entropy(torch.clamp(input, 0, 1), target, reduction='none')
        return torch.log((torch.exp(x) + torch.exp(-x)) / 2.0)
